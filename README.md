...
回測（2021–2025）：邏輯迴歸 AUC 0.64、前 100 覆蓋 43%；規則 0.63／42%；GBDT 0.61／44%；按次數 0.61／43%。五年 walk-forward（2021–2025）。門檻採抽檢邏輯（寧可錯殺）：「中」≥ 10% 為人力跑得完的最低點，五年匯總 recall 0.72、precision 0.17；「高」≥ 22% recall 0.46。AUC 選模型、recall 定門檻。
ROI 重播：同樣 312 次／季人力，規則排程命中次年被罰園 40–53%，輪流稽查 25–28%。

## 個資

園名與負責人姓名為公開登記資料，畫面顯示；被裁罰的個人（行為人）姓名在架構層擋住（API 與問答只讀 `v_*` view）。demo DB 另清空電話與地址；Google 評論（公開評論，含原文）保留。
