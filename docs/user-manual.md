...
- **等級**：依「12 個月再犯機率」的絕對門檻分高／中／低（預設高 ≥ 22%、中 ≥ 10%，設定頁可改）。門檻採抽檢邏輯：寧可錯殺，「中」設在人力跑得完的最低點，五年回測 recall 0.72。無裁罰紀錄的園列「無紀錄」、已停辦列「停辦」，都排在最後、不出分數。
...
| 燈 | 紅 | 黃 | 意義 |
|---|---|---|---|
| 人事費率（人事費 ÷ 支出合計） | > 全體第 90 百分位 | > 第 75 百分位 | 人事費占支出比例偏高 |
| 每核定名額收入（收入 ÷ 核定人數） | < 第 10 百分位 | < 第 25 百分位 | 招生不足 |
...
- 每個版本的回測卡有五個測試年（2021–2025）逐年 AUC，與**混淆矩陣**（中／高兩種門檻）、accuracy、balanced accuracy、precision、recall、specificity、F1、Cohen's κ、Brier、log loss。看法：AUC 選模型；recall 定門檻（抽檢寧可錯殺）；precision 是白跑的代價；accuracy 要對照基準率 15% 看。
